...
class WXRImporter extends \PostmagThemes\WPContentImporter2\WXRImporter {
	...
	public function __construct( $options = array() ) {
		...
		// print_r($_SESSION["imprter_user_id"]);exit;
		// Set current user to $mapping variable.
		// Fixes the [WARNING] Could not find the author for ... log warning messages.

		// $current_user_obj =	get_userdata($_SESSION["imprter_user_id"]);
		// print_r($current_user_obj);
		// exit;
		if(isset($_SESSION["imprter_user_id"])){
			$importUserID = get_userdata($_SESSION["imprter_user_id"]);
			// $userLoginIm = $importUserID->user_login;
			// $userIdIm = $importUserID->ID;
